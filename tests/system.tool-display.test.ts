...
import { formatDuration, formatParams, formatToolCall, Spinner, truncate, formatToolResult, formatToolCycle } from "../src/tool-display.js";
...
describe("truncate", () => {
  ...
  describe("formatToolCall", () => {
    it("should show tool name and params", () => {
      const result = stripAnsi(formatToolCall("read_file", { file_path: "test.txt" }));
  ...
  describe("formatToolCycle", () => {
    it("should combine header, timing, and result", () => {
      const result = stripAnsi(
        formatToolCycle("read_file", { file_path: "test.txt" }, "file contents", 150)
      );
      expect(result).toContain("read_file");
      expect(result).toContain("test.txt");
