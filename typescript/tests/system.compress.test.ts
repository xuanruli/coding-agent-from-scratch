...
import type { LLMProvider, Message, ChatResponse } from "../src/llm/types.js";

// Mock provider that returns a fixed summary
function mockProvider(summaryText = "Summary of the conversation."): LLMProvider {
  return {
    chat: vi.fn(async () => ({
      ...
      const msgs: Message[] = [
        {
          role: "assistant",
          content: [
            { type: "tool_use", id: "1", name: "read_file", input: { file_path: "test.txt" } },
          ],
        },
        {
          role: "user",
